...
function ParseConfig() {
  ...
  ParamAddValue(param, catname, "Wait");
  ...
  while (aktline < config_split.length) {
    for (var cat in category) {
      ...
      if (
        config_split[aktline].trim().toUpperCase() == zw1 ||
        config_split[aktline].trim().toUpperCase() == zw2
      ) {
  ...
  if (
    param["DataLogging"]["DataLogActive"]["enabled"] == false &&
    param["DataLogging"]["DataLogActive"]["value1"] == ""
  ) {
    param["DataLogging"]["DataLogActive"]["found"] = true;
    param["DataLogging"]["DataLogActive"]["enabled"] = true;
    param["DataLogging"]["DataLogActive"]["value1"] = "true";
  ...
  if (
    param["DataLogging"]["DataFilesRetention"]["enabled"] == false &&
    param["DataLogging"]["DataFilesRetention"]["value1"] == ""
  ) {
    param["DataLogging"]["DataFilesRetention"]["found"] = true;
    param["DataLogging"]["DataFilesRetention"]["enabled"] = true;
    param["DataLogging"]["DataFilesRetention"]["value1"] = "3";
...
function ParamExtractValue(
  ...
  if (
    _linesplit[0].toUpperCase() == _paramname.toUpperCase() &&
    _linesplit.length > _anzvalue
  ) {
        ...
        if (abc["name"] == "default") {
          for (_num in NUMBERS) {
            ...
            if (NUMBERS[_num][_catname][paramname]["found"] == false) {
              ...
              for (
                var j = 1;
                j <= _param[_catname][paramname]["anzParam"];
                ++j
              ) {
...
function getCamConfig() {
  ...
  if (!param["AutoTimer"]["Wait"]["found"]) {
    param["AutoTimer"]["Wait"]["found"] = true;
    param["AutoTimer"]["Wait"].value1 = "5";
  }
      ...
      if (param[cat][name]["Numbers"]) {
        for (_num in NUMBERS) {
          ...
          for (var j = 1; j <= param[cat][name]["anzParam"]; ++j) {
            if (
              !(typeof NUMBERS[_num][cat][name]["value" + j] == "undefined")
            ) {
        ...
        if (NUMBERS[_roi]["digit"].length > 0) {
          for (var _roiddet in NUMBERS[_roi]["digit"]) {
            text =
              NUMBERS[_roi]["name"] +
              "." +
              NUMBERS[_roi]["digit"][_roiddet]["name"];
        ...
        if (NUMBERS[_roi]["analog"].length > 0) {
          for (var _roiddet in NUMBERS[_roi]["analog"]) {
            text =
              NUMBERS[_roi]["name"] +
              "." +
              NUMBERS[_roi]["analog"][_roiddet]["name"];
