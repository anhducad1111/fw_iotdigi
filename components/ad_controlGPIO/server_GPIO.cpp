...
GpioPin::GpioPin(gpio_num_t gpio, const char* name, gpio_pin_mode_t mode, gpio_int_type_t interruptType, uint8_t dutyResolution, bool httpEnable) 
{
    _gpio = gpio;
    _name = name; 
    _mode = mode;
...
//    ESP_LOGD(TAG, "readConfig - Start 3");
    ...
    while (configFile.getNextLine(&line, disabledLine, eof) && !configFile.isNewParagraph(line))
    {
        ...
        if ((splitted[0].rfind("IO", 0) == 0) && (splitted.size() >= 6))
        {
            ...
            GpioPin* gpioPin = new GpioPin(gpioNr, gpioName, pinMode, intType,dutyResolution, httpEnabled);
